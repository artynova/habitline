...
def test_habit_analysis_filter(analysis_filter: HabitAnalysisFilter, analysis: HabitAnalysis, expected: bool):
    """
...
@pytest.mark.parametrize(["analysis_order", "analysis", "expected_key"], [
    ...
    pytest.param(HabitAnalysisOrder.by_created_at(False),
                 make_mock_analysis(created_at=datetime(2026, 5, 16, 21, 13, 20)), datetime(2026, 5, 16, 21, 13, 20),
                 id="by_created_at"),
...
def test_habit_analysis_order(analysis_order: HabitAnalysisOrder, analysis: HabitAnalysis, expected_key: any):
    """
...
@pytest.mark.parametrize(["habit", "analysis_range", "now", "expected_result"], [
    ...
    pytest.param(
        make_mock_habit(periodicity=Periodicity.DAILY, created_at=datetime(2026, 5, 14, 13, 30, 16), completions=()),
    ...
    pytest.param(
        make_mock_habit(periodicity=Periodicity.WEEKLY, created_at=datetime(2026, 5, 13, 13, 30, 16), completions=()),
        AnalysisRange(None, None), datetime(2026, 5, 14, 18, 12, 30),
        HabitAnalysisResult(0, 0, 0.0, True),
        id="weekly_new"),
    pytest.param(
...
def test_analyse_one(habit: Habit, analysis_range: AnalysisRange, now: datetime,
...
def test_analyse_many(habits: list[Habit], analyser: Callable[[Habit, AnalysisRange, datetime], HabitAnalysis],
...
def test_aggregate(habits: list[Habit], analyser: Callable[[Habit, AnalysisRange, datetime], HabitAnalysis],

...
from habitline.repository import HabitRepository, Periodicity, HabitNameTakenException, HabitIdentifier, \
    HabitNotFoundException, periodicity_to_stored, datetime_to_stored, Habit
...
def make_select_habit_row(id: int, name: str, periodicity: Periodicity, created_at: datetime,
    ...
    """
    return id, name, periodicity_to_stored(periodicity), datetime_to_stored(created_at), datetime_to_stored(
        completed_at) if completed_at is not None else None
...
class TestHabitRepository:
    ...
    def test_create_success(self, mock_connection: Mock, periodicity: Periodicity):
        ...
        assert set(insert_parameters) == {name, periodicity_to_stored(periodicity), datetime_to_stored(created_at)}
